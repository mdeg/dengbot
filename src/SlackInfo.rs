...
impl SlackInfo {
    pub fn new(resp: ::slack::api::rtm::StartResponse) -> Self {
        let mut channels = resp.channels.expect("No channel list returned").into_iter();
        ...
        let meta_channel_id = channels
            .find(|channel| channel.name.as_ref().expect("No listen channel name found") == "dengsmeta")
        ...
        let users = resp.users.expect("No users returned on connection");

        SlackInfo {
            users, listen_channel_id, meta_channel_id
        }
    }
}